    ...
    function generateUpdate(updateData){
        ...
        for(var column in updateData) {
            if (updateData.hasOwnProperty(column)) {
                ...
                // if have equal thats mean the command is also value
                // like example : 'usage=coalesce(usage,0)+1'
                // and is also already add to columnNames, so next please ;-)
                var commandHaveEqual = column.indexOf('=') > -1;
                if(commandHaveEqual){
                    continue;
                }
